"""
Synthetic-themes - multi-pass lifecycle demo.

The single-pass demo (`run_demo.py`) proves the pipeline runs. This script
shows the *belief revision* part: expectations change over time, and the
...
Two calibration passes:
    pass A:  expectations from era1 train     -> calibrate on era2 test
    pass B:  expectations from era1+era2 train -> calibrate on era3 test
...
(via `predicted_class` carried in Hypothesis.direction) and emits a real
transition: strengthened, weakened, contradicted, retired, or inverted.

The full L2 expectation (narrative / state / score / read) lives in
Hypothesis.extras and renders on each card. The lifecycle transition is
driven by the calibrated accuracy of `predicted_class` — the *bridge* — not
by the richer fields.

Requires OPENAI_API_KEY in env. Roughly 16 LLM expectation calls plus one
batched embedding call per run.
...
def _calibrate_pass(
    *,
    ...
    gen:           LLMTeamsExpectationGenerator,
    timestamp:     datetime,
...
def main() -> None:
    ...
    out = render_region_cards_html(
        title        = "Synthetic themes - multi-pass lifecycle (pass A -> pass B)",
