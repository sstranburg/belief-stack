...
**Status:** Working draft — paper-in-progress. Iterated as additional substrates, models, and economic measurements land.
...
We interpret the combined findings as evidence that maintained state functions as a distinct planning substrate rather than a better summarization strategy. The load-bearing operation is the substrate transformation — filtering to currently-held beliefs, dedup-clustering, ranking — not the rendering of richer warrant or lifecycle metadata into the planner's context. We sketch the architecture this evidence implies: *one substrate, asymmetric consumer projections* — a sparse state projection for planners and a rich inspection surface for humans.

Results are bounded to operational workflow substrate (Claude Code session logs), a single model family (`gpt-4o-2024-08-06`), a single budget regime (~285 tokens), and fixtured belief extraction. Cross-substrate transfer, model variance, and end-to-end maintenance economics remain open questions.
...
A natural architectural alternative is to maintain workflow state explicitly: derive a representation of currently-held beliefs from the event stream, project that representation into the planner's context, and let the planner reason against a current view of the world rather than reconstructing one from raw history. We refer to this maintained representation as a *belief stack*, with the architectural pattern of *claim + warrant + lifecycle* as its substrate-side contract.
...
The paper proceeds as follows. §2 places the work in the context of related approaches to LLM state management. §3 describes the substrate and the scoring methodology. §4 reports the three experimental phases. §5 develops the architectural interpretation the evidence supports. §6 discusses limitations. §7 identifies the open empirical directions.
...
### 5.3 The architecture this evidence implies

Combining v0.4a.1 and v0.4a.2, the architecture splits more cleanly than the v0.3 spec articulated:

| Layer | Properties | Empirical anchor |
|---|---|---|
| **Substrate** | Rich. Claims + warrants + lifecycle preserved. Rule engine runs upstream. Filtering, dedup-clustering, ranking, lifecycle-state computation are all part of the substrate's contract. | Load-bearing per v0.4a.2 (A′ < B). Without the substrate transformation, compression of raw context cannot recover the planning lift. |
| **Planner projection** | Sparse. Bare `belief_type :: claim` per active cluster, dedup-ranked, budget-bounded. ~208 tokens reaches 97.3% correctness on operational substrate at the v0.4a cell. Adding warrant or lifecycle fields to the projection does not add measurable correctness and may add noise. | Arm C is the empirical Pareto reference. |
| **Human inspection projection** | Rich. Full warrant chains, lifecycle timelines, audit trails. What an operator needs to understand at any past turn what the system believed and why. | Not measured in v0.4a; structurally distinct from the planner's optimal shape; remains a design commitment based on the inspection use case rather than an empirical finding. |

The architectural commitment is *one substrate, asymmetric consumer projections*. The same lifecycle-aware substrate produces two consumer-specific projections, and the optimal shape of each is structurally distinct: sparse for planners (empirically), rich for humans (by design). The dual-consumer pattern is preserved, but the consumers' optimal projections are more different than the v0.3 framing suggested.

### 5.4 Why the planner doesn't need rich renderings (a candidate mechanism)

The v0.4a.1 result invites a hypothesis about *why* the bare projection wins. At small budgets on a single-next-action planning task, the planner appears to need the *names* of currently-held beliefs (what is true now) more than it needs the *evidence chain* (why each belief is held). The substrate machinery already determines which beliefs are currently held by filtering retired and contradicted entries and dedup-clustering by claim. The planner downstream consumes the result: *here are the beliefs currently in force; choose the next action consistent with them.*

When warrant fields are added to the projection, the budget per belief grows, fewer beliefs fit, and the additional fields appear to add noise the planner does not productively use at single-next-action resolution. Whether this hypothesis generalizes — to longer planning horizons, larger budgets, or substrates where evidence chains carry more diagnostic information — is the open question §7 takes up.
...
In rough priority order, refined by what v0.4a's findings revealed:

1. **Cross-substrate replication.** The most important next experiment. Replicate the v0.4a six-arm program on a second operational substrate (DevOps incident logs, customer-support trajectories) to test whether Arm C–style projection generalizes beyond Claude Code session logs. After v0.4a.2 ruled out the compression confound, the single largest live risk to the v0.4a interpretation is the single-substrate caveat.

2. **Budget-floor scan.** A small experiment scanning Arm C's budget from approximately 50 tokens up to the v0.4a cap of 285 tokens. v0.4a sets an upper bound on the *minimum-sufficient* projection at 208 tokens; the actual floor — and whether the descent is gradual or cliff-shaped — is not measured.

3. **End-to-end economics.** v0.4a measured planning consumption only. The substrate-side maintenance cost — tokens, latency, and dollars per evidence event for write-path rule engines — is unmeasured. The crossover question: how many planning queries amortize one substrate update? With Arm C's small per-query cost, the breakeven is likely favorable, but the question deserves direct measurement.

4. **Model variance.** Replicate across at least one frontier-class alternative model and at least one smaller / cheaper model, holding substrate and budget constant. The architectural claim earns considerably stronger if the projection trade-off transfers across the model field.

5. **Warrant utility at write time.** v0.4a found warrant fields unhelpful in the planner-facing projection. Warrants may remain load-bearing in the substrate's *write path* — deciding which beliefs to maintain, computing decay, scoring authority-based conflict resolution. This is a separate research direction; v0.4a's findings do not bear on it.

6. **Human-inspection projection.** The asymmetric-projections claim is empirically anchored on the planner side. The human-inspection side is currently a design commitment, not a measured one. Future work could measure human debugging performance, audit-task accuracy, or trust-calibration outcomes under different inspection projections.

7. **Multi-step planning.** v0.4a's single-next-action task is friendly to compact maintained state. Multi-step planning — where chains of consequence matter — may rebalance the projection trade-off.
...
- **v0.1 (2026-06-04)** — initial working draft. Covers v0.3, v0.4a.1, v0.4a.2. Open directions include cross-substrate replication (most important), budget-floor scan, end-to-end economics, model variance, warrant write-time utility, human-inspection measurement, multi-step planning.
