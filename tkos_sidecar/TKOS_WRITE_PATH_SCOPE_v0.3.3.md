# TKOS Write-Path Sidecar — Scope v0.3.3

**Date:** v0.2 → v0.2.1 → v0.3.1 → v0.3.2 → v0.3.3 (this version, 2026-06-06)
**Status:** LOCKED at v0.3.3 — implementation-ready at protocol-completion level. Supersedes v0.3.2.

**v0.3.2 → v0.3.3 patches (three scope-level fixes in this doc):**
- **Fix I — Finalization timing tightened.** §6.1a — the v0.3.2 "task_completion + 60s inactivity" auto-finalization rule was too aggressive: Codex rollouts can contain multiple task cycles, and a new task appended later would invalidate the stored final hashes. v0.3.3: finalization is **explicit only** — triggered automatically at the end of batch replay (the file is fixed in size by then) or by explicit CLI command (`tkos finalize <session>`) when the user is sure no more lines will be appended. There is no auto-finalize trigger based on event content. A separate doc-level note ("when to finalize a live Codex session") is appended to §6.1a as guidance.
- **Fix II — HTTP endpoint payload contract locked.** §1.3 + §7 — the endpoint takes **one raw rollout-JSONL line per POST**, NOT a parsed event. The server parses and routes the line via `ingest_source_line()`. v0.3.2 §1.3 said "one event per POST" in one place and "one raw line" in another. v0.3.3 locks the canonical payload as raw line; acceptance test 1 updated to match ("POST one valid rollout JSONL line").
- **Fix III — `line_hash_chain` maintenance in atomic ingestion.** §6.1 — v0.3.2's `ingest_source_line()` pseudocode did not show the `line_hash_chain` update. v0.3.3 explicitly adds it as part of the same transaction that inserts the `raw_lines` row. On replay no-op (existing row matches), the chain is NOT extended (because the line is already in the chain from the original ingest). On mismatch, `SourceMutationError` aborts before any chain update.

Cross-doc patches in this v0.3.3 revision cycle: Fix IV (`pipeline_running` dispatch path) → `RULES_SPEC_v0.3.2.md`; Fix V (`report_ready` depends on successful result) → `RULES_SPEC_v0.3.2.md`; Fix VI (synthetic `action_blocked` count semantics) → `TKOS_READ_PATH_MIGRATION_v0.3.3.md`. INTEGRATION_PATTERN stays at v0.1.3.
...
- HTTP ingestion endpoint accepting **one raw rollout JSONL line per POST** (v0.3.3 — fix II). The endpoint body is the bytes of a single JSONL line (UTF-8, no trailing newline). The server parses and routes the line via `ingest_source_line()` per §6.1.
...
def ingest_source_line(raw_line: bytes, session_id, source_line_number):
    ...
    # 1a. Extend line_hash_chain in the same transaction (v0.3.3 — fix III)
    #     The chain MUST include every line (mapped, ignored-known, unrecognized).
    #     This is the per-line value of H_n = sha256(H_{n-1} || raw_line_n_bytes).
    #     Replay no-ops above skip this update because the chain already includes
    #     this line from the original ingest.
    prev_chain = SELECT COALESCE(line_hash_chain, '') FROM session_status
        WHERE session_id=:session_id
    new_chain = sha256(prev_chain.encode() + raw_line)
    UPDATE session_status SET line_hash_chain=:new_chain
        WHERE session_id=:session_id
...
`finalize_session()` is triggered (v0.3.3 — fix I: explicit-only in live mode):

- **Batch mode (`tkos replay <trace.jsonl>`):** automatically after the last line of the rollout has been ingested. The file is fixed in size by then; no race condition.
- **Live mode:** **explicitly only** — via CLI (`tkos finalize <session_id> <rollout_path>`) when the user determines no more lines will be appended. There is no event-content-based auto-finalize trigger. Codex rollouts can contain multiple task cycles (`task_start` → `task_completion` → ... → `task_start` → ...); finalizing after the first `task_completion` would invalidate the stored hashes when a subsequent task appends new lines.
...
**Guidance — when to finalize a live Codex session:**

The session is "closed" only when no more lines will be appended. Suggested workflow:
- After the Codex desktop app or CLI session has been fully exited.
- After a quiescence window comfortably longer than the longest expected gap between tasks (default suggestion: 24 hours; configurable).
- Explicitly by the user via `tkos finalize` when they know the session is done.

Calling `finalize_session()` on a still-open session, then ingesting more lines, **invalidates the stored hashes**. The implementation MUST raise `SessionAlreadyFinalizedError` if `ingest_source_line()` is called for a session whose `capture_ended_at IS NOT NULL`. There is no re-open semantic in v0.3.3; if a session needs more lines after finalize, drop and re-ingest from scratch.
...
1. **Single-line ingest (v0.3.3 — fix II).** POST one valid rollout JSONL line representing a `function_call` (which the adapter normalizes to a `tool_call` event); verify a row appears in `raw_lines` with `category=mapped`, a row appears in `events` with all §4.2 fields populated, and `line_hash_chain` has been advanced; verify the transaction committed atomically.
...
- **Q1. HTTP framing (v0.3.3 — fix II clarified):** one **raw rollout JSONL line** per POST. The server parses and dispatches via `ingest_source_line()`. The endpoint does not accept pre-parsed events.
